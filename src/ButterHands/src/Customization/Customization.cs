...
namespace ButterHands.Customizations
{
	public class Customization
	{
		...
		private void FVRQuickBeltSlot_Update(MonoMod.Cil.ILContext il)
		{
			...
			while (DoesAnotherQBSlotSetColorExist(c) && counter <= 4)
			{
				...
				(ConfigEntry<Color> colorCfg, ConfigEntry<float> intensityCfg) = counter switch
				{
					0 => (cfg.SpawnlockColor, cfg.SpawnlockIntensity), // Spawnlock QB
					1 => (cfg.HarnessColor, cfg.HarnessIntensity), // Harness QB
					_ => (cfg.BaseColor, cfg.BaseIntensity) // Base QB
				};

				Color color = colorCfg.Value;
				float intensity = intensityCfg.Value;

				// New RGBA values
				c.Emit(OpCodes.Ldc_R4, color.a * intensity);
				c.Emit(OpCodes.Ldc_R4, color.g * intensity);
				c.Emit(OpCodes.Ldc_R4, color.b * intensity);
				c.Emit(OpCodes.Ldc_R4, color.a);
